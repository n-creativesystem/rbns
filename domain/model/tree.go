...
func (p *permissionsImpl) Get() []string {
	return append(make([]string, 0, len(p.values)), p.values...)
}
...
func (n *Node) Add(method, path string, permissions ...string) {
	root := n.trees.get(method)
	if root == nil {
		root = new(node)
		root.fullPath = "/"
		n.trees = append(n.trees, methodTree{method: method, root: root})
	}
	p := &permissionsImpl{
		values: append(make([]string, 0, len(permissions)), permissions...),
	}
	root.addRoute(path, p.Get)
}

func (n *Node) Has(method, path string) bool {
	return len(n.Get(method, path)) > 0
}
...
func (n *node) addRoute(path string, handlers IPermissions) {
	fullPath := path
	n.priority++
...
func (n *node) insertChild(path string, fullPath string, handlers IPermissions) {
	for {
