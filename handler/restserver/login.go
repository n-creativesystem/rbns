...
func (s *HTTPServer) GetOAuthProvider(c ncsfw.Context) error {
...
func (s *HTTPServer) OAuthLogin(c ncsfw.Context) error {
	r := c.Request()
	ctx := r.Context()
	...
	loginUser := buildExternalUserInfo(ctx, token, userInfo, name)
	...
	if err := bus.Dispatch(ctx, cmd); err != nil {
		s.log.ErrorWithContext(ctx, err, "database save error")
...
func (s *HTTPServer) handleOAuthLoginError(c ncsfw.Context, err response.ErrorResponse) {
	s.log.ErrorWithContext(c.Request().Context(), err, "login")
	u := path.Join(s.Cfg.RootURL.Path, "/login")
	s.Redirect(c, u)
}

func (s *HTTPServer) Logout(c ncsfw.Context) error {
	r := c.Request()
	session, err := s.store.Get(r, OauthSessionName)
	if err != nil {
		return err
...
func buildExternalUserInfo(ctx context.Context, token *oauth2.Token, userInfo *social.BasicUserInfo, name string) *model.LoginUser {
	...
	if err := bus.Dispatch(ctx, &query); err != nil {
		...
		user.SetOAuthName(name).SetTenant("")
	} else {
		user = *query.Result
	}
	return &user
